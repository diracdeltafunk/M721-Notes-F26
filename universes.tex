...
%TODO This is taken from a short note I started writing once but never finished. Perhaps eventually it will be included as an appendix to these lecture notes.

\section{Classes, Sets, and Universes}
In an introduction to category theory, one often learns that a category $\catC$ consists of a \emph{collection} of objects (together with morphisms, composition operations, etc. satisfying some axioms). Often in mathematics, the word ``collection'' is synonymous with the word ``set'', but not here! What is meant precisely by the word ``collection'' in the definition of a category depends slightly on how one wishes to formalize their set-theoretic foundations for category theory, and there are generally two popular approaches here. We will work in ZFC set theory. For readers already comfortable with the notions of small/large categories and Grothendieck universes, this section may be safely skipped.

\subsection{Classes}

A famous argument of Russel shows that there is no set of sets, i.e.
...
\begin{proof}
    ...
    \[B \coloneq \{x \in A : x \notin x\}.\]
    Now by construction, $B \in B \iff B \notin B$, which is a contradiction.
\end{proof}
...
Now the key point is that the axiom schema of restricted comprehension allows us to build \emph{subsets} $\{x \in A : \varphi(x)\}$ of any \emph{pre-existing set} $A$, but it does \emph{not} allow us to construct new sets which are not subsets of a pre-existing sets! In particular, the axiom schema does not \textbf{not} allow us to interpret
\[\{x : x \notin x\}\]
...
Now consider the class $\{x : x \neq x\}$. One can just as easily prove that $\forall y (y \notin \{x : x \neq x\})$, since this is shorthand for $\forall y(\lnot (y \neq y))$, which is true. In other words, we should think of $\{x : x \neq x\}$ as the ``empty class''. There is also the empty set $\varnothing$\footnote{The existence of $\varnothing$ follows (in ZFC) from the axiom of infinity together with the axiom schema of specification.}, and we note that
...
\end{equation}
for this class, abusing notation since this is not precisely in the form $\{x : \phi(x)\}$. It's not a big deal, because there is a canonical procedure to rewrite \eqref{eq:class of groups 2} in the standard form \eqref{eq:class of groups 1}.
...
\end{proposition}
\begin{proof}
    First, suppose $\catC$ is small, i.e. $\mor \catC$ is a set. Then for all $x,y \in \catC$ we have
...
\begin{definition}
    ...
    \begin{enumerate}
        \item $U$ is transitive, i.e. $$\forall x \forall y (x \in y \in U \implies x \in U),$$
        \item $U$ is closed under forming pairs, i.e. $$\forall x,y \in U (\{x,y\} \in U),$$
        \item $U$ is closed under forming power sets, i.e. $$\forall x \in U (\mathcal{P}(x) \in U),$$ where $\mathcal{P}(x)$ denotes the power set of $x$,
        \item $U$ is closed under unions, i.e. $$\forall I \in U \forall f : I \to U \left(\bigcup_{i \in I} f(i) \in U\right),$$
...
We note now that this setup allows us to define the free cocompletion functor \[Y : \Cat \to \CAT\] which sends any $U_1$-small category $\catC \in \ob \Cat$ to the category of presheaves $\Fun(\catC^\mathrm{op}, \Set)$, which will be $U_2$-small. Since $\Cat$ is a full subcategory of $\CAT$, we can use this to prove a generalized version of the Yoneda lemma!

\begin{proposition}
    Let $i$ denote the inclusion of the full subcategory $\Cat \subseteq \CAT$. There is a natural transformation $y$ from $i$ to the free cocompletion functor $Y := \Fun({(-)}^\mathrm{op}, \Set)$ given componentwise by the Yoneda embedding\footnote{One should carefully define the Yoneda embedding and note that it is well-defined: since $\catC$ is $U_1$-small, each hom-set $\Hom_\catC(x,y)$ is indeed an object of $\Set$.}
    \[y_\catC : \catC \to \Fun(\catC^\mathrm{op}, \Set) = Y(\catC).\]
    Moreover, the isomorphisms of the Yoneda lemma
    \[\Psi_{\catC, x, F} : \Hom_{Y(\catC)}(y_\catC(x), F) \xrightarrow{\cong} F(x)\]
    assemble into a natural isomorphism between the functors
    \[(x,F) \mapsto F(x)\]
\end{proposition}